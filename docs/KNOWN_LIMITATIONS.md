...
The PadChest-GR numbers (`results/external/`) apply the sealed MS-CXR models without any retraining or tuning to the PadChest-GR test split, restricted to sentences whose label maps onto an MS-CXR finding (strict mapping 194 sentences, dominated by cardiomegaly 99; extended mapping adds 85 opacity-type sentences as lung opacity). PadChest-GR has no edema or pneumonia sentences, consolidation and pneumothorax have four sentences each, and no method was retrained on this set. MedRPG and TransVG (MS-CXR-1444 seeds 13/42/2026) were applied in the same zero-shot, one-box manner on a Linux workstation (`results/external/workstation_run_20260910.json`); MedGrounder could not be re-applied (no trained checkpoint on that machine). The values therefore compare MS-CXR-trained models under domain shift and describe the transfer of the re-ranker gain; they are not a PadChest-GR benchmark ranking. The ClueGround numbers were reproduced on two machines (RTX 4090 and RTX 3090 Ti) with seed-wise differences below 0.006 from non-deterministic half-precision inference; the manuscript reports the 4090 run. Atelectasis and opacity-type sentences transfer poorly. The 16-bit PNGs were min-max scaled to 8 bit per image; PadChest-GR sentences are English translations of Spanish reports and the deterministic parser was not adapted to them.
