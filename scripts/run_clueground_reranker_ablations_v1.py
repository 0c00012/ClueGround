#!/usr/bin/env python
"""Component and RAD-DINO query ablations for the learned re-ranker path.

Positive control: the full path is re-run with every downstream parameter
frozen to the sealed run (scorer retrained deterministically from the cached
train table, alpha, single-route calibration cache and set parameters); its
eval Coverage must match the sealed value exactly.

Query ablation (inference-time masking of the 91-D RAD-DINO query, checkpoint
fixed): the masked RAD-DINO box changes the ``dino_iou`` feature and the
fusion input; scorer, alpha, calibration and set parameters stay frozen.

Component ablation:
  * ``rad_dino_only``  one RAD-DINO box per group (no YOLO)
  * ``yolo_only``      YOLO candidates without RAD-DINO anywhere: the scorer is
                       retrained without ``dino_iou``, alpha and calibration are
                       re-selected on val (its own fair configuration)
  * ``no_reranker``    the sealed canonical hybrid-v4 (alpha = 0) - reported
                       from its own sealed run, not recomputed here
"""
...
DEFAULT_FULL = PROJECT_ROOT / "experiments" / "clueground_canonical_learned_reranker_noaux_assets_v1"
DEFAULT_OUTPUT = PROJECT_ROOT / "experiments" / "clueground_reranker_ablations_v1"
QUERY_VARIANTS = tuple(v for v in fa.QUERY_VARIANTS if v != "full_query")
METRICS = ("coverage_iou", "exact_union_iou", "set_f1_optimal_0_3", "set_f1_optimal_0_5", "mean_pred_count")


def log(message: str) -> None:
    print(f"[ablation {time.strftime('%H:%M:%S')}] {message}", flush=True)


def dino_pixels(norm_by_gid: dict[str, np.ndarray], rows: list[dict[str, Any]]) -> dict[str, list[float]]:
    out = {}
    for row in rows:
        gid = str(row["group_id"])
        if gid in norm_by_gid:
            out[gid] = ybase.norm_to_xyxy(np.asarray(norm_by_gid[gid], dtype=np.float32), float(row["image_width"]), float(row["image_height"]))
    return out


class FixedSetParams:
    """Context manager that freezes hybrid_v4.tune_set_params to sealed values."""
...
def run_seed(seed: int, full_root: Path, output_root: Path, device: torch.device) -> list[dict[str, Any]]:
    ...
    exact.PROTOCOL_ROOT = canonical.PROTOCOL_ROOT
    exact.MULTI_SOURCE_ROOT = canonical.UPSTREAM_ROOT

    seed_root = full_root / f"seed_{seed}"
    status = json.loads((full_root / "learned" / f"RUN_STATUS_s{seed}.json").read_text(encoding="utf-8"))
    alpha = float(status["learned_alpha"])
    sealed_cov = float(status["coverage_iou"])
    selection = json.loads((seed_root / "scorer_selection.json").read_text(encoding="utf-8"))
    cols = list(selection["features"])
    run_root = full_root / "learned" / "multibox_1444" / f"seed_{seed}"
    ...
    if max_diff > 1e-9:
        raise RuntimeError(f"seed {seed}: scorer retrain does not reproduce sealed scores (max diff {max_diff})")
    log(f"seed {seed}: scorer reproduced (max diff {max_diff:.1e}); alpha={alpha}; sealed coverage {sealed_cov:.6f}")
    ...
    def run_fixed(name: str, dino_norm: dict[str, dict[str, np.ndarray]] | None) -> dict[str, Any]:
        ...
        for split in ("val", "eval"):
            if dino_norm is not None:
                ...
                scored[split] = score_split(split, dino_pixels(dino_norm[split], rows[split]))
            else:
        ...
        with FixedSetParams(set_params, set_grid):
            result = exact.run_protocol(ctx, output_root / name, quick=False, retune_single_full_val=True, separate_multi_route_params=True, calibration_cache_root=full_root / "learned")
        return result
    ...
    control_cov = float(control["coverage_iou"])
    passed = abs(control_cov - sealed_cov) < 1e-9
    log(f"seed {seed}: positive control coverage {control_cov:.6f} sealed {sealed_cov:.6f} pass={passed}")
    results.append({"table": "query", "variant": "full_query", "seed": seed, "positive_control_pass": passed, **metric_row(control)})
    if not passed:
        raise RuntimeError("positive control failed; downstream is not frozen")

    # query ablation
    for variant in QUERY_VARIANTS:
        dino_norm = fa.multi_dino_variant(seed, variant, device, 64)
        result = run_fixed(variant, dino_norm)
        results.append({"table": "query", "variant": variant, "seed": seed, **metric_row(result)})
        log(f"seed {seed} query {variant}: coverage {result['coverage_iou']:.4f} f1@.5 {result['set_f1_optimal_0_5']:.4f}")

    # component: RAD-DINO only
    ...
    log(f"seed {seed} component rad_dino_only: coverage {summary['coverage_iou']:.4f}")

    # component: YOLO only (no RAD-DINO anywhere; own fair selection on val)
    ...
    log(f"seed {seed} component yolo_only: alpha={alpha_nd} coverage {result['coverage_iou']:.4f}")
...
def main() -> None:
    ...
    rr.write_json(args.output_root / "FINAL_STATUS.json", {"status": "complete", "full_root": str(args.full_root), "seeds": args.seeds, "positive_control_pass": bool(frame[frame.variant == "full_query"]["positive_control_pass"].all())})
